...
### User Story 3 — Перепривязка / отвязка / удаление (Priority: P2)

Как пользователь, я хочу иметь возможность **отвязать** чек от платежа, **перепривязать** к другому платежу и/или **удалить** чек, чтобы исправлять ошибки и поддерживать чистые данные.

**Acceptance Scenarios**:

1. **Given** чек привязан к платежу, **When** я нажимаю “Отвязать”, **Then** связь “чек → платеж” удаляется, а чек возвращается в состояние “не привязан”.
2. **Given** чек привязан к платежу A, **When** я выбираю платеж B и подтверждаю, **Then** связь переносится на платеж B (с аудитом кто/когда).
3. **Given** чек загружен, **When** я удаляю чек, **Then** метаданные помечаются удалёнными (soft delete) и файл становится недоступен.

---

### User Story 4 — Подготовка к ручной линковке платежа к продукту (Priority: P3)
...
- Файлы нельзя хранить на локальном диске контейнера (Render) — после деплоя/рестарта они могут пропасть. Нужно внешнее персистентное хранилище.
- Дубли чеков:
  - пользователь случайно загрузил один и тот же файл несколько раз;
  - один и тот же чек сфотографирован несколько раз (похожие изображения, но разные файлы);
  - один документ привязали к платежу, а затем повторно загрузили его в другой месяц (нужно понять, как это выявлять/помечать).
...
- **FR-010**: UI таба “Чеки” MUST быть оптимизирован под мобильные устройства: одна кнопка загрузки, понятный прогресс, кликабельные крупные строки кандидатов.
- **FR-011**: Система MUST хранить файлы чеков/инвойсов в **персистентном хранилище**, не зависящем от деплоя/рестарта (например, object storage).
- **FR-012**: Система MUST раскладывать файлы по годам/месяцам (для удобства администрирования/ретеншена), например: `receipts/YYYY/MM/<receiptId>/<filename>`.
- **FR-013**: Система MUST поддерживать ретеншен/удаление: возможность удалить чек (soft delete метаданных + удаление/скрытие файла).
- **FR-014**: Система MUST уметь определять явные дубли загрузок (минимум по `sha256` файла) и помечать их как дубликаты, не создавая “новый независимый чек”.
- **FR-015**: Система MUST предоставлять способ работы с дублями:
  - показать “этот чек уже загружен” + ссылку на оригинал,
  - разрешить пользователю удалить дубль (или пометить как дубль),
  - при необходимости — перепривязать чек к другому платежу (через unlink/link).
- **FR-016**: Система MUST определить поведение для многостраничных PDF в V1 (минимум: обрабатывать 1-ю страницу и явно показывать это в результате).
...
- **NFR-004**: API SHOULD поддерживать “мобильный” режим: быстрый ответ `202 Accepted` + polling через `GET`, чтобы не держать длинный запрос на слабой сети.
- **NFR-005**: Хранилище файлов MUST гарантировать сохранность при деплоях и рестартах приложения (без зависимости от локальной ФС контейнера).
- **NFR-006**: Ссылки на файлы SHOULD быть signed/временные (или отдача через backend), чтобы не делать документы публичными.
- **NFR-007**: Дедупликация SHOULD быть быстрой и не требовать загрузки файла в БД (хэш считается на backend при приёме файла).
