...
## Duplicates research (how to handle)

Типовые дубли:

- **Точный дубль файла**: пользователь загрузил один и тот же файл повторно.
  - Надежный детектор: `sha256` (hash от bytes файла).
- **Похожий, но не identical**: тот же чек, но переснятый (разные байты).
  - Это требует perceptual hashing / ML и в V1 можно не делать автоматически.

Практичный V1:

- Дедуп только по `sha256`.
- UI/логика позволяют:
  - увидеть, что файл уже загружен,
  - перейти к оригиналу,
  - не плодить независимые сущности (или пометить дубль через `duplicate_of`).
